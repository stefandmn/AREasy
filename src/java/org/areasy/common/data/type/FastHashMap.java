 ...
 *
 * @version $Id: FastHashMap.java,v 1.3 2008/05/20 06:52:55 swd\stefan.damian Exp $
 */
public class FastHashMap extends HashMap
{
	...
	public boolean equals(Object o)
	{
		...
		if (fast)
		{
			if (mo.size() != map.size()) return (false);
			...
			while (i.hasNext())
			{
				...
				if (value == null)
				{
					if (!(mo.get(key) == null && mo.containsKey(key))) return (false);
				}
				else
				{
					if (!value.equals(mo.get(key))) return (false);
				}
			}
		...
		private class CollectionViewIterator implements Iterator
		{
			...
			public void remove()
			{
				...
				if (fast)
				{
					synchronized (FastHashMap.this)
					{
						if (expected != map) throw new ConcurrentModificationException();
