 ...
 *
 * @version $Id: ObjectArrayIterator.java,v 1.2 2008/05/14 09:32:40 swd\stefan.damian Exp $
 */
public class ObjectArrayIterator implements Iterator, ResettableIterator
{
	...
	public ObjectArrayIterator(Object array[], int start, int end)
	{
		...
		if (start < 0) throw new ArrayIndexOutOfBoundsException("Start index must not be less than zero");
		if (end > array.length) throw new ArrayIndexOutOfBoundsException("End index must not be greater than the array length");
		if (start > array.length) throw new ArrayIndexOutOfBoundsException("Start index must not be greater than the array length");
		if (end < start) throw new IllegalArgumentException("End index must not be less than start index");
	...
	{
		if (this.array != null) throw new IllegalStateException("The array to iterate over has already been set");
