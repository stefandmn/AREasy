 ...
 *
 * @version $Id: CursorableLinkedList.java,v 1.2 2008/05/14 09:32:35 swd\stefan.damian Exp $
...
public class CursorableLinkedList extends AbstractLinkedList implements Serializable
{
	...
	{
		// We take this opportunity to clean the cursors list of WeakReference objects to garbage-collected cursors.
		for (Iterator it = cursors.iterator(); it.hasNext();)
		{
			WeakReference ref = (WeakReference) it.next();
			if (ref.get() == null) it.remove();
		}
	...
	protected void broadcastNodeChanged(Node node)
	{
		...
		while (it.hasNext())
		{
			...
			if (cursor == null) it.remove(); // clean up list
				else cursor.nodeChanged(node);
		}
	}
	...
	public static class Cursor extends AbstractLinkedList.LinkedListIterator
	{
		...
		public int nextIndex()
		{
			if (!nextIndexValid)
			{
				if (next == parent.header) nextIndex = parent.size();
				else
				{
					int pos = 0;
		...
		{
			if (node.previous == current) next = node;
				else if (next.previous == node) next = node;
					else nextIndexValid = false;
		}
		...
		{
			if (!valid) throw new ConcurrentModificationException("Cursor closed");
		}
