 ...
 *
 * @version $Id: AbstractHashedMap.java,v 1.3 2008/05/14 09:32:30 swd\stefan.damian Exp $
 */
public class AbstractHashedMap extends AbstractMap implements IterableMap
{
	...
	protected AbstractHashedMap(int initialCapacity, float loadFactor)
	{
		...
		if (initialCapacity < 1) throw new IllegalArgumentException("Initial capacity must be greater than 0");
		if (loadFactor <= 0.0f || Float.isNaN(loadFactor)) throw new IllegalArgumentException("Load factor must be greater than 0");
	...
	public Object get(Object key)
	{
		...
		while (entry != null)
		{
			if (entry.hashCode == hashCode && isEqualKey(key, entry.key)) return entry.getValue();
	...
	public boolean containsKey(Object key)
	{
		...
		while (entry != null)
		{
			if (entry.hashCode == hashCode && isEqualKey(key, entry.key)) return true;
			entry = entry.next;
		}
	 ...
	 * Removes the specified mapping from this map.
	 *
	 * @param value the mapping to remove
	 * @return true if the value was deleted from all recorded enties.
	 */
	public boolean removeByValue(Object value)
	{
		...
		for (int i = 0; i <  data.length; i++)
		{
			...
			while (entry != null)
			{
				...
				{
					removeMapping(entry, i, previous);
					deleted = true;
				}
				else
				{
	...
	protected HashEntry getEntry(Object key)
	{
		...
		while (entry != null)
		{
			if (entry.hashCode == hashCode && isEqualKey(key, entry.key)) return entry;
			entry = entry.next;
		}
	...
	{
		if (previous == null) data[hashIndex] = entry.next;
	...
	protected void checkCapacity()
	{
		if (size >= threshold)
		{
			...
			if (newCapacity <= MAXIMUM_CAPACITY) ensureCapacity(newCapacity);
		}
	}
