...
import org.areasy.common.data.type.set.CompositeSet;
 ...
 *
 * @version $Id: CompositeMap.java,v 1.2 2008/05/14 09:32:30 swd\stefan.damian Exp $
 */
public class CompositeMap implements Map
{
	...
	public synchronized void addComposited(Map map) throws IllegalArgumentException
	{
		for (int i = composite.length - 1; i >= 0; --i)
		{
			Collection intersect = CollectionUtility.intersection(this.composite[i].keySet(), map.keySet());
			if (intersect.size() != 0)
			{
