...
import org.areasy.common.data.type.iterator.EmptyMapIterator;
 ...
 *
 * @version $Id: Flat3Map.java,v 1.2 2008/05/14 09:32:30 swd\stefan.damian Exp $
 */
public class Flat3Map implements IterableMap, Serializable, Cloneable
{
	...
	{
		if (delegateMap != null) return delegateMap.get(key);
	...
	public int size()
	{
		if (delegateMap != null) return delegateMap.size();
	...
	{
		if (delegateMap != null) return delegateMap.containsKey(key);
	...
	{
		if (delegateMap != null) return delegateMap.containsValue(value);
	...
	{
		if (delegateMap != null) return delegateMap.put(key, value);
	...
	{
		if (delegateMap != null) return delegateMap.remove(key);
	...
	{
		if (delegateMap != null) return delegateMap.mapIterator();

		if (size == 0) return EmptyMapIterator.INSTANCE;
	...
	static class FlatMapIterator implements MapIterator, ResettableIterator
	{
		...
		public Object next()
		{
			if (!hasNext()) throw new NoSuchElementException(AbstractHashedMap.NO_NEXT_ENTRY);
		...
		public void remove()
		{
			if (!canRemove) throw new IllegalStateException(AbstractHashedMap.REMOVE_INVALID);
		...
		{
			if (!canRemove) throw new IllegalStateException(AbstractHashedMap.GETKEY_INVALID);
		...
		{
			if (!canRemove) throw new IllegalStateException(AbstractHashedMap.GETVALUE_INVALID);
		...
		{
			if (canRemove) return "Iterator[" + getKey() + "=" + getValue() + "]";
	...
	public Set entrySet()
	{
		if (delegateMap != null) return delegateMap.entrySet();
	...
	static class EntrySet extends AbstractSet
	{
		...
		{
			if (parent.delegateMap != null) return parent.delegateMap.entrySet().iterator();

			if (parent.size() == 0) return EmptyIterator.INSTANCE;
	...
	static class EntrySetIterator implements Iterator, Map.Entry
	{
		...
		public void remove()
		{
			if (canRemove == false) throw new IllegalStateException(AbstractHashedMap.REMOVE_INVALID);
		...
		public Object getKey()
		{
			if (canRemove == false) throw new IllegalStateException(AbstractHashedMap.GETKEY_INVALID);
		...
		public Object getValue()
		{
			if (canRemove == false) throw new IllegalStateException(AbstractHashedMap.GETVALUE_INVALID);
		...
		public Object setValue(Object value)
		{
			if (canRemove == false) throw new IllegalStateException(AbstractHashedMap.SETVALUE_INVALID);
