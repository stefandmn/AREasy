...
import org.areasy.common.data.type.collection.CompositeCollection;
 ...
 *
 * @version $Id: CompositeSet.java,v 1.2 2008/05/14 09:32:37 swd\stefan.damian Exp $
 */
public class CompositeSet extends CompositeCollection implements Set
{
	 ...
	 * @see org.areasy.common.data.type.collection.CompositeCollection.CollectionMutator
	 * @see SetMutator
	 */
	public synchronized void addComposited(Collection c)
	{
		...
		{
			Set set = (Set) i.next();
			Collection intersects = CollectionUtility.intersection(set, c);
			if (intersects.size() > 0)
			{
				...
				if (CollectionUtility.intersection(set, c).size() > 0)
				{
