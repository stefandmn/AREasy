...
import org.areasy.common.parser.excel.biff.formula.FunctionNames;
...
public final class WorkbookSettings
{
	 ...
	 * The logger
	 */
	private static Logger logger = LoggerFactory.getLog(WorkbookSettings.class);
	 ...
	 * Drawings deactivated using -Dparser.excel.nodrawings=true on the JVM command line
	 * Activated by default or by using -Dparser.excel.nodrawings=false on the JVM command
	 * line
	 */
	 ...
	 * Names deactivated using -Dparser.excel.nonames=true on the JVM command line
	 * Activated by default or by using -Dparser.excel.nonames=false on the JVM command
	 * line
	 */
	 ...
	 * GC deactivated using -Dparser.excel.nogc=true on the JVM command line
	 * Activated by default or by using -Dparser.excel.nogc=false on the JVM command line
	 */
	 ...
	 * performance reasons.  It can be deactivated using -Dparser.excel.norat=true on
	...
	public WorkbookSettings()
	{
		...
		try
		{
			drawingsDisabled = Boolean.getBoolean("parser.excel.nodrawings");
			namesDisabled = Boolean.getBoolean("parser.excel.nonames");
			gcDisabled = Boolean.getBoolean("parser.excel.nogc");
			rationalizationDisabled = Boolean.getBoolean("parser.excel.norat");
			mergedCellCheckingDisabled = Boolean.getBoolean("parser.excel.nomergedcellchecks");
			formulaReferenceAdjustDisabled = Boolean.getBoolean("parser.excel.noformulaadjust");
			propertySetsDisabled = Boolean.getBoolean("parser.excel.nopropertysets");
			ignoreBlankCells = Boolean.getBoolean("parser.excel.ignoreblanks");
			cellValidationDisabled = Boolean.getBoolean("parser.excel.nocellvalidation");
			autoFilterDisabled = !Boolean.getBoolean("parser.excel.autofilter");

			// autofilter currently disabled by default
			useTemporaryFileDuringWrite = Boolean.getBoolean("parser.excel.usetemporaryfileduringwrite");

			String tempdir =System.getProperty("parser.excel.temporaryfileduringwritedirectory");
			if (tempdir != null) temporaryFileDuringWriteDirectory = new File(tempdir);
		...
		try
		{
			if (System.getProperty("parser.excel.lang") == null || System.getProperty("parser.excel.country") == null)locale = Locale.getDefault();
				else locale = new Locale(System.getProperty("parser.excel.lang"), System.getProperty("parser.excel.country"));

			if (System.getProperty("parser.excel.encoding") != null) encoding = System.getProperty("parser.excel.encoding");
		}
		catch (SecurityException e)
		{
			logger.warn("Error accessing system properties.", e);
