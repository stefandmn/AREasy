...
public final class CompoundFile extends BaseCompoundFile
{
	...
	public CompoundFile(byte[] d) throws BiffException
	{
		...
		{
			if (data[i] != IDENTIFIER[i]) throw new BiffException(BiffException.unrecognizedOLEFile);
		}
		...
		if (numExtensionBlocks != 0) bbdBlocks = (BIG_BLOCK_SIZE - BIG_BLOCK_DEPOT_BLOCKS_POS) / 4;
		...
		for (int j = 0; j < numExtensionBlocks; j++)
		{
			...
			int blocksToRead = Math.min(numBigBlockDepotBlocks - bbdBlocks, BIG_BLOCK_SIZE / 4 - 1);
		...
		while (offset < rootEntry.length)
		{
			...
			if (ps.name == null || ps.name.length() == 0)
			{
				if (ps.type == ROOT_ENTRY_PS_TYPE)
				{
					...
					logger.warn("Property storage name for " + ps.type + " is empty - setting to " + ROOT_ENTRY_NAME);
				}
				else
				{
					if (ps.size != 0)
			...
			if (ps.name.equalsIgnoreCase(ROOT_ENTRY_NAME)) rootEntryPropertyStorage = ps;
			offset += PROPERTY_STORAGE_BLOCK_SIZE;
		}

		if (rootEntryPropertyStorage == null) rootEntryPropertyStorage = (PropertyStorage) propertySets.get(0);
	}
	...
	{
		PropertyStorage ps = findPropertyStorage(streamName, rootEntryPropertyStorage);

		// Property set can't be found from the direct hierarchy, so just search on the name
		if (ps == null)
		{
			ps = getPropertyStorage(streamName);
		}

		if (ps.size >= SMALL_BLOCK_THRESHOLD || streamName.equalsIgnoreCase(ROOT_ENTRY_NAME))
		{
	...
	public byte[] getStream(int psIndex) throws BiffException
	{
		...
		if (ps.size >= SMALL_BLOCK_THRESHOLD || ps.name.equalsIgnoreCase(ROOT_ENTRY_NAME))
		{
			return getBigBlockStream(ps);
		}
		else
	...
	private PropertyStorage findPropertyStorage(String name, PropertyStorage base)
	{
		...
		while (prev.previous != -1)
		{
			...
			if (prev.name.equalsIgnoreCase(name)) return prev;
		}
		...
		while (next.next != -1)
		{
			...
			if (next.name.equalsIgnoreCase(name)) return next;
		}
	...
	private PropertyStorage getPropertyStorage(String name) throws BiffException
	{
		...
		if (multiple) logger.warn("Found multiple copies of property set " + name);

		if (!found) throw new BiffException(BiffException.streamNotFound);
	...
	private byte[] getBigBlockStream(PropertyStorage ps)
	{
		...
		if (ps.size % BIG_BLOCK_SIZE != 0) numBlocks++;
		...
		if (block != -2 && count == numBlocks) logger.warn("Property storage size inconsistent with block chain.");
	...
	private byte[] readData(int bl) throws BiffException
	{
		...
		for (; blockCount <= bigBlockChain.length && block != -2; blockCount++)
		{
			...
			System.arraycopy(data, pos, entry, oldEntry.length, BIG_BLOCK_SIZE);

			if (bigBlockChain[block] == block) throw new BiffException(BiffException.corruptFileFormat);
	 ...
	 *
	 * @param index the property set index
