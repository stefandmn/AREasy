...
import org.areasy.common.parser.excel.biff.PaletteRecord;
import org.areasy.common.parser.excel.biff.drawing.DrawingGroup;
import org.areasy.common.parser.excel.biff.drawing.MsoDrawingGroupRecord;
		...
		implements ExternalSheet, WorkbookMethods
{
	 ...
	 * The logger
	 */
	private static Logger logger = LoggerFactory.getLog(WorkbookParser.class);
	 ...
	 */
	private DefaultSheet lastSheet;
	...
	public Sheet getSheet(int index)
	{
		...
		lastSheet = (DefaultSheet) sheets.get(index);
	...
	protected void parse() throws BiffException, PasswordException
	{
		...
		while (bof != null && getNumberOfSheets() < boundsheets.size())
		{
			...
			{
				// Read the sheet in
				DefaultSheet s = new DefaultSheet(excelFile,
						sharedStrings,
						formattingRecords,
						bof,
						workbookBof,
		...
		for (Iterator it = localNames.iterator(); it.hasNext();)
		{
					...
					nr.getBuiltInName() == BuiltInName.PRINT_TITLES)
			{
				...
				DefaultSheet s = (DefaultSheet) sheets.get(nr.getSheetRef() - 1);
				s.addLocalName(nr);
			}
		}
	}
