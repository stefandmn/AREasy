...
import org.areasy.common.parser.excel.biff.drawing.Origin;
...
import org.areasy.common.parser.excel.format.RGB;
 ...
 * A writable workbook
 */
public class DefaultWritableWorkbook extends WritableWorkbook
		implements ExternalSheet, WorkbookMethods
{
	/**
	 * The logger
	 */
	private static Logger logger = LoggerFactory.getLog(DefaultWritableWorkbook.class);
	/**
	 ...
	 * @throws IOException
	 */
	public DefaultWritableWorkbook(OutputStream os, boolean cs, WorkbookSettings ws)
			throws IOException
	{
		super();
	 ...
	 * @throws IOException
	 */
	public DefaultWritableWorkbook(OutputStream os,
								Workbook w,
								boolean cs,
								WorkbookSettings ws) throws IOException
	{
		...
		if (wp.getExternalSheetRecord() != null)
		{
			...
			org.areasy.common.parser.excel.read.biff.SupbookRecord[] readsr = wp.getSupbookRecords();
			supbooks = new ArrayList(readsr.length);

			for (int i = 0; i < readsr.length; i++)
			{
				org.areasy.common.parser.excel.read.biff.SupbookRecord readSupbook = readsr[i];
		...
		{
			org.areasy.common.parser.excel.read.biff.NameRecord[] na = wp.getNameRecords();
	...
	private WritableSheet createSheet(String name, int index,
									  boolean handleRefs)
	{
		WritableSheet w = new DefaultWritableSheet(name,
				outputFile,
				formatRecords,
				sharedStrings,
				settings,
	...
	public void write() throws IOException
	{
		...
		// the workbook
		DefaultWritableSheet wsi = null;
		for (int i = 0; i < getNumberOfSheets(); i++)
		{
			wsi = (DefaultWritableSheet) getSheet(i);
		...
		if (!sheetSelected)
		{
			wsheet = (DefaultWritableSheet) getSheet(0);
		...
		for (int i = 0; i < getNumberOfSheets(); i++)
		{
			...
			if (((DefaultWritableSheet) sheets.get(i)).isChartOnly())
			{
				br.setChartOnly();
			}
	...
	private void copyWorkbook(Workbook w)
	{
		...
		Sheet s = null;
		DefaultWritableSheet ws = null;
		for (int i = 0; i < numSheets; i++)
		{
			s = w.getSheet(i);
			ws = (DefaultWritableSheet) createSheet(s.getName(), i, false);
			ws.copy(s);
		}
	}
	...
	public void copySheet(int s, String name, int index)
	{
		...
		DefaultWritableSheet ws = (DefaultWritableSheet) createSheet(name, index);
		ws.copy(sheet);
	}
	...
	private void rationalize()
	{
		...
		{
			wsi = (DefaultWritableSheet) sheets.get(i);
	 ...
	 */
	public org.areasy.common.parser.excel.read.biff.BOFRecord getWorkbookBof()
	{
		return null;
	}
	...
	public int getExternalSheetIndex(String sheetName)
	{
		...
		Iterator i = sheets.iterator();
		int sheetpos = 0;
		DefaultWritableSheet s = null;

		while (i.hasNext() && !found)
		{
			s = (DefaultWritableSheet) i.next();
	 ...
	 */
	void columnInserted(DefaultWritableSheet s, int col)
	{
	 ...
	 */
	void columnRemoved(DefaultWritableSheet s, int col)
	{
		int externalSheetIndex = getExternalSheetIndex(s.getName());
		for (Iterator i = rcirCells.iterator(); i.hasNext();)
		{
	 ...
	 */
	void rowInserted(DefaultWritableSheet s, int row)
	{
		int externalSheetIndex = getExternalSheetIndex(s.getName());
	 ...
	 */
	void rowRemoved(DefaultWritableSheet s, int row)
	{
		int externalSheetIndex = getExternalSheetIndex(s.getName());
		for (Iterator i = rcirCells.iterator(); i.hasNext();)
		{
	...
	public WritableSheet importSheet(String name, int index, Sheet sheet)
	{
		...
		((DefaultWritableSheet) ws).importSheet(sheet);
