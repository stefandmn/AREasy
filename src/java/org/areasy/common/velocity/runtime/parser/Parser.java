...
import org.areasy.common.velocity.runtime.directive.Macro;
import org.areasy.common.velocity.runtime.directive.MacroParseException;
 ...
 *
 * @version $Id: Parser.java,v 1.2 2008/05/29 14:04:13 swd\stefan.damian Exp $
 */
public class Parser implements ParserTreeConstants, ParserConstants
{
	/** the logger */
	private static Logger logger = LoggerFactory.getLog(Parser.class.getName());
	 ...
	 */
	public Parser(RuntimeService rs)
	{
		/*
		 ...
		 */
		velcharstream = new VelocityCharStream(new ByteArrayInputStream("\n".getBytes()), 1, 1);
	...
	public SimpleNode parse(Reader reader, String templateName) throws ParseException
	{
		...
		{
			logger.error("Parser error:  #macro() : " + templateName + " : " + mee.getMessage());
			logger.debug("Exception", mee);

			throw new ParseException(mee.getMessage());
		}
		catch (ParseException pe)
		{
			logger.error("Parser exception: '" + templateName + "' template : " + pe.getMessage());
		...
		{
			logger.error("Parser error: '" + templateName + "' template : " + e.getMessage());
	...
	private String escapedDirective(String strImage)
	{
		...
		else if (rsvc.isVelocityMacro(strDirective.substring(1), currentTemplateName))
		{
			bRecognizedDirective = true;
		}
		else
	...
	final public void Escape() throws ParseException
	{
		...
		try
		{
			...
			else if (rsvc.isVelocityMacro(t.next.image.substring(1), currentTemplateName))
			{
				control = true;
			}
	...
	final public SimpleNode Directive() throws ParseException
	{
		...
		try
		{
			...
			if (d == null)
			{
				...
				isVM = rsvc.isVelocityMacro(directiveName, currentTemplateName);
