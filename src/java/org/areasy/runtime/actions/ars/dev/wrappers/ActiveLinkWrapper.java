...
/**
 * Active Link definition object implementation.
 *
 */
public class ActiveLinkWrapper extends AbstractWrapper implements ObjectWrapper
{
	/**
	 ...
	 */
	public ActiveLinkWrapper(DefinitionAction action)
	{
		init(action);
	}
	...
	public List<String> find(Date since)
	{
		try
		{
			return getServerConnection().getContext().getListActiveLink(since != null ? new Timestamp(since).getValue() : 0);
		}
	...
	{
		return getServerConnection().getContext().getActiveLink(name);
	}
	...
	{
		ObjectBase object = new ActiveLink();
		object.setName(name);
	...
	{
		return StructItemInfo.ACTIVE_LINK;
	}
	...
	{
		return DefinitionAction.TYPE_ACTIVELINKS;
	}
}
