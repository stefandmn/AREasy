...
public class DevelopmentPackageObject
{
	/** Library logger */
	protected static Logger logger = LoggerFactory.getLog(DevelopmentPackageObject.class);
	...
	public DevelopmentPackageObject(String data[])
	{
	...
	public DevelopmentPackageObject(StructItemInfo info)
	{
		if(info == null) throw new RuntimeException("Invalid input data structure for Development Package process");

		setAction(BaseDevelopment.ACT_MODIFY);
		setSignature( BaseDevelopment.getObjectSignatureByType(info.getType()) );
	...
	{
		StructItemInfo info = new StructItemInfo(BaseDevelopment.getObjectTypeIdBySignature(signature), getObjectName(), null);
