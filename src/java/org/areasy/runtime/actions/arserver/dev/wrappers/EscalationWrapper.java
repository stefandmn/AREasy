...
/**
 * Escalation definition object implementation.
 *
 */
public class EscalationWrapper extends AbstractWrapper implements ObjectWrapper
{
	/**
	 ...
	 */
	public EscalationWrapper(DefinitionAction action)
	{
		init(action);
	}
	...
	public List<String> find(Date since)
	{
		try
		{
			return getServerConnection().getContext().getListEscalation(since != null ? new Timestamp(since).getValue() : 0);
		}
	...
	{
		return getServerConnection().getContext().getEscalation(name);
	}
	...
	{
		ObjectBase object = new Escalation();
		object.setName(name);
	...
	{
		return StructItemInfo.ESCALATION;
	}
	...
	{
		return DefinitionAction.TYPE_ESCALATIONS;
	}
}
