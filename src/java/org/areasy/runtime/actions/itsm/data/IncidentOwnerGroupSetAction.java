...
/**
 * Change owner group of one incident using merge operation. Owner person is left unchanged.
 ...
 *
 */
public class IncidentOwnerGroupSetAction extends AbstractAction
{
	...
	public void run() throws AREasyException
	{
		...
		incident.setOwnerCompany(toCompany);
		incident.setOwnerOrganisation(toOrg);
		incident.setOwnerGroup(toGroup);
		incident.setOwnerGroupId(sgroup.getEntryId());

		incident.merge(getServerConnection(), Constants.AR_MERGE_ENTRY_DUP_MERGE);

		RuntimeLogger.add("Owner Group of incident " + incidentNumber + " has been set to " + toGroup);
	}
}
