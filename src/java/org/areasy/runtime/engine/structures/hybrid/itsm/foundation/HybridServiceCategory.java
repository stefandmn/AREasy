...
import org.areasy.runtime.engine.structures.data.itsm.foundation.ServiceCategory;
...
/**
 * This is a data structure who manages <Code>ServiceCategory</code> entities that includes all related sub-structures,
 * and each transaction of the main entity will handle also the parts.
 */
public class HybridServiceCategory extends ServiceCategory implements AbstractHybrid
{
	private Configuration config = null;

	public HybridServiceCategory()
	{
		super();
	}
	 ...
	 */
	public HybridServiceCategory getInstance()
	{
		return new HybridServiceCategory();
	}
	...
	public void relate(ServerConnection arsession, Configuration config) throws AREasyException
	{
		...
		if(exists())
		{
			if(config.getBoolean("companyrelation", true))
			{
				...
				searchRel.setFormName("CFG:Service Catalog Assoc");
				searchRel.setAttribute(1000000096, getAttributeValue(1));
				...
				for(int i = 0; relations != null && i < relations.size(); i++)
				{
					...
					addPartKeyPairWithPrefix("companyrelation", item, 1000000096, 1);
				}
			}
		}
	}
	...
	public void transfer(CoreItem target, Map<Integer,Object> map, ServerConnection arsession, Configuration config) throws AREasyException
	{
		...
		if(target instanceof HybridServiceCategory)
		{
			HybridServiceCategory sgroup = (HybridServiceCategory)target;
